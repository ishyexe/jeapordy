...
let NUM_CATEGORIES = [];
// this function will retrieve the api with the random categories (i included a count of 100 so that i would always have a lot of options for the 6 random categories)
...
function getRandomNum() {
  let mySet = new Set();
  while (mySet.size <= 5) {
    //so that it's alway's 6 categories
...
async function getCategory(catId) {
  try {
    ...
    const categoryObj = {
      title: title,
      clues: shuffleArray(clueArray).slice(0, 5),
    };
    //return categoryObj;
